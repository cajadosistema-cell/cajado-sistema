    ...
    try {
      ...
      //                     valor_pago?: number, data_pagamento?: 'YYYY-MM-DD' }
        ...
        if (tipoAlvo === 'cartao') {
          ...
          let mesRefExec = acao.dados.mes_referencia
          if (!mesRefExec) {
            const resRef = await resolverMesRefPendente(supabase, 'pagamentos_imoveis', 'imovel_id', imovelAch.id, imovelAch.dia_vencimento, dataPag, imovelAch.data_aquisicao, imovelAch.parcelas_pagas, imovelAch.parcelas_total, imovelAch.periodicidade)
            mesRefExec = resRef.mesRef
          }
          ...
          const { data: pagExistente } = await (supabase.from('pagamentos_imoveis') as any)
            .select('id, status').eq('imovel_id', imovelAch.id).eq('mes_referencia', mesRefExec).maybeSingle()
          if (pagExistente?.status === 'pago') {
            setMensagens(prev => [...prev, { id: `pago-${Date.now()}`, role: 'ai' as const,
              texto: `ℹ️ O boleto do **${imovelAch.titulo}** (${mesRefExec}) **já estava marcado como pago** — não lancei nada de novo pra não duplicar o débito.` }])
            setAcaoStatus(msgId, acaoIdx, 'saved')
            window.dispatchEvent(new CustomEvent('elena:lancamento-salvo'))
          ...
          let qVe = (supabase.from('veiculos') as any)
            .select('id, titulo, marca, modelo, valor_parcela, parcelas_pagas, parcelas_total, vencimento_dia')
          ...
          let mesRefExecVe = acao.dados.mes_referencia
          if (!mesRefExecVe) {
            const resRefVe = await resolverMesRefPendente(supabase, 'pagamentos_veiculos', 'veiculo_id', veiculoAch.id, veiculoAch.vencimento_dia, dataPag, null)
            mesRefExecVe = resRefVe.mesRef
          }
          ...
          const { data: pagVeExistente } = await (supabase.from('pagamentos_veiculos') as any)
            .select('id, status').eq('veiculo_id', veiculoAch.id).eq('mes_referencia', mesRefExecVe).maybeSingle()
          if (pagVeExistente?.status === 'pago') {
            setMensagens(prev => [...prev, { id: `pago-${Date.now()}`, role: 'ai' as const,
              texto: `ℹ️ A parcela do **${nomeVeiculo}** (${mesRefExecVe}) **já estava marcada como paga** — não lancei nada de novo.` }])
            setAcaoStatus(msgId, acaoIdx, 'saved')
            window.dispatchEvent(new CustomEvent('elena:lancamento-salvo'))
            window.dispatchEvent(new CustomEvent('elena:patrimonio-updated'))
            return
          ...
          const { error: errPagVe } = await (supabase.from('pagamentos_veiculos') as any).upsert({
            veiculo_id: veiculoAch.id, empresa_id: empresaIdConf, mes_referencia: mesRefExecVe,
          ...
          setMensagens(prev => [...prev, { id: `pago-${Date.now()}`, role: 'ai' as const,
            texto: `✅ Parcela do **${nomeVeiculo}** (${mesRefExecVe}) marcada como paga via **${contaOrigemNomeVe}** — parcela ${Math.min(ppVeAtual + 1, ptVeTotal)}/${ptVeTotal}.${avisoDebitoVe}` }])
          window.dispatchEvent(new CustomEvent('elena:patrimonio-updated'))
      ...
      // ── RESUMO MENSAL ESTRUTURADO (formato tabela — Sr. Max) ──
        ...
        // 01/08/2026: acabou a janela fixa de 6 meses. Os meses em aberto agora
        // são derivados do contrato (data_aquisicao + parcelas_pagas +
        // periodicidade) por calcularParcelasEmAberto, e podem ser bem mais
        // antigos que 6 meses — Intermediárias Ciacci tem parcela trimestral
        // vencida desde 2025-02. Por isso a consulta abaixo perdeu o piso.
        ...
        const [
          ...
          // Faturas do mês (valor_previsto = prévia manual, valor_fechado = fatura real)
          (supabase.from('faturas_cartoes') as any)
            .select('conta_id, valor_previsto, valor_fechado, status, data_pagamento')
            .eq('user_id', uid)
        ...
        if (cartoesLista.length === 0) {
          ...
          for (const cartao of cartoesLista) {
            ...
            const valorFechado = fatura ? Number(fatura.valor_fechado) || 0 : 0
            const valorPrevisto = fatura ? Number(fatura.valor_previsto) || 0 : 0
            // Valor de referência: fatura fechada > prévia manual > zero
            const valorRef = valorFechado > 0 ? valorFechado : valorPrevisto
        ...
        // `venc` é 'AAAA-MM-DD': ordena a tabela e é o que aparece na coluna
        // Vencimento (substituiu as duas colunas antigas, Dia e Mês Ref — foi
        // o "Mês Ref 09/2026" dentro do resumo de agosto que confundiu o Max).
        // `futuro` = vence depois deste mês: APARECE na tabela, mas fica fora
        // da somatória. Pedido do Max, 03/08: mostrar tudo, somar só o mês.
        type LinhaBoleto = { desc: string; venc: string; valor: number; parcela: string; status: string; pago: boolean; atrasado: boolean; futuro: boolean }
        ...
        ;(imoveisData || []).filter((im: any) => im.valor_parcela != null).forEach((im: any) => {
          ...
          // MESMA função que o card do Patrimônio usa (patrimonio-pagamentos.ts).
          // A parcela nº N vence em data_aquisicao + N × passo(periodicidade); as
          // pagas são as primeiras parcelas_pagas. pagamentos_imoveis só refina.
          // Enquanto as duas telas tinham lógicas próprias elas divergiam.
          const calc = calcularParcelasEmAberto(
            {
              dataAquisicao: im.data_aquisicao,
              parcelasPagas: im.parcelas_pagas,
          ...
          // Contrato quitado não aparece no resumo de forma alguma.
          if (calc.quitado) return

          const descBase = im.construtora ? `${im.titulo} (${im.construtora})` : im.titulo
          const desc = calc.semDataAquisicao ? `${descBase} ⚠️ sem data de aquisição` : descBase

          // Nada em aberto: o imóvel não sumiu, mas a linha tem de dizer a
          // verdade sobre QUANDO vence. `proximo_vencimento` é a âncora
          // (migration 079): preenchida, ela manda.
          if (calc.emAberto.length === 0) {
            const ancora = String(im.proximo_vencimento || '').slice(0, 10)
            const mesAncora = ancora.slice(0, 7)
            // 12/08/2026: O PAGAMENTO DO MÊS MANDA MAIS QUE A ÂNCORA.
            // Desde que o pagamento passou a AVANÇAR a âncora, um imóvel pago
            // em agosto ficava com âncora em setembro — e a linha pulava para
            // setembro como 🟢 Futuro, fora do total. Resultado: a parcela que
            // o Sr. Max acabou de pagar desaparecia do resumo do mês em que ele
            // pagou, e o VALOR PAGO do mês ficava menor que a realidade.
            // Regra: se existe pagamento registrado para o mês do resumo, a
            // linha é DESTE mês, com o vencimento deste mês. A âncora só decide
            // quando não há pagamento no mês.
            const pagoNoMes = porMes.get(mesRef)
            const temPagamentoNoMes = pagoNoMes?.status === 'pago' || pagoNoMes?.status === 'parcial'
            const futuro = !temPagamentoNoMes && !!ancora && mesAncora > mesRef
            const pagRef = temPagamentoNoMes ? pagoNoMes : porMes.get(futuro ? mesAncora : mesRef)
          ...
          calc.emAberto.forEach(pa => {
            const statusPg = porMes.get(pa.mesRef)?.status || 'pendente'
            linhasBoletos.push({
              desc,
        ...
        if (linhasBoletos.length === 0) {
          ...
          texto += `| Vencimento | Compromisso | Valor | Parcela | Status |\n`
          texto += `|------------|-------------|------:|---------|--------|\n`
        ...
        // TODOS os contratos aparecem na tabela (pedido do Max, 03/08), mas só
        ...
        if (invNaTabela.length > 0) {
          ...
          // primeiro contrato virava o nome da seção inteira e confundia.
          texto += `📈 **INVESTIMENTOS**\n`
          texto += `| Vencimento | Contrato | Parcela | Valor | Status |\n`
          texto += `|------------|----------|---------|------:|--------|\n`
