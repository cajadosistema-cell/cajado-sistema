  ...
  return (
    <>
      ...
      {showMore && (
        ...
        >
          <div
            className="absolute bottom-[65px] left-0 right-0 bg-[#0a0d16] border-t border-zinc-800 rounded-t-2xl p-4"
            onClick={e => e.stopPropagation()}
          >
            <div className="w-10 h-1 bg-zinc-700 rounded-full mx-auto mb-4" />
            <p className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold mb-3">Mais módulos</p>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0a0d16]/95 backdrop-blur-md border-t border-zinc-800 pb-safe">
        <div className="flex items-center justify-around px-1 py-2">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? `${item.activeColor} scale-105` : 'text-zinc-500 hover:text-zinc-300'
                )}
              >
          ...
          <button
            ...
            className={cn(
              'relative flex flex-col items-center justify-center p-2 rounded-xl transition-all duration-200 min-w-0 flex-1',
              showMore ? 'text-amber-400 scale-105' : 'text-zinc-500 hover:text-zinc-300'
            )}
          >
