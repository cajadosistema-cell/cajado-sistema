...
export function TabPermissoes() {
  ...
  if (carregando) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="text-sm text-fg-disabled">Carregando permissões...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-sm font-semibold text-fg-secondary mb-1">🔐 Controle de Acesso por Role</h2>
      ...
      <div className="space-y-3">
        {perfis.length === 0 ? (
          <p className="text-sm text-fg-disabled text-center py-8">Nenhum usuário cadastrado ainda.</p>
        ) : (
          perfis.map(perfil => {
            ...
            return (
              <div key={perfil.id} className="bg-page border border-border-subtle rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
                {/* Info do usuário */}
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-fg">{perfil.nome}</p>
                    <p className="text-xs text-fg-tertiary">{perfil.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  {info ? (
                    ...
                    <span className="text-xs text-fg-disabled px-2.5 py-1 rounded-lg border border-border-subtle">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {salvando === perfil.id && (
                    <span className="text-xs text-fg-tertiary">Salvando...</span>
                  )}
                </div>
              </div>
            )
      ...
      <details className="group">
        <summary className="text-xs text-fg-tertiary hover:text-fg-secondary cursor-pointer select-none list-none flex items-center gap-1">
          <span className="group-open:rotate-90 transition-transform">▶</span>
          Ver detalhes de permissões por role
        </summary>
        <div className="mt-3 bg-page border border-border-subtle rounded-xl p-4 overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border-subtle">
                <th className="text-left text-fg-tertiary pb-2 pr-4">Role</th>
                <th className="text-left text-fg-tertiary pb-2">Permissões</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(ROLE_LABELS).map(([slug, info]) => (
                <tr key={slug} className="border-b border-border-subtle/50 last:border-0">
                  ...
                  <td className="py-2">
                    <div className="flex flex-wrap gap-1">
                      {ROLE_PERMISSOES[slug as RoleSlug].map(p => (
                        <span key={p} className="bg-muted text-fg-secondary px-1.5 py-0.5 rounded text-[9px] font-mono">
                          {p}
                        </span>
                      ))}
                    </div>
