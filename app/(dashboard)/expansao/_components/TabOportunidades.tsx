  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-sm font-semibold text-zinc-200">🚀 Pipeline de Inovação & Expansão</h2>
      ...
      {ativas.length === 0 && !showForm ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8"><EmptyState message="Sem iniciativas no pipeline" /></div>
      ) : (
          ...
          {KANBAN_STATUS.map(col => (
            <div key={col.id} className={cn('min-w-[280px] w-full max-w-[320px] rounded-xl border p-3 flex flex-col snap-center', col.color)}>
              <div className="flex items-center justify-between mb-4 px-1">
                <h3 className="text-xs font-bold text-zinc-300">{col.label}</h3>
                <span className="text-[10px] text-zinc-500 bg-zinc-900/50 px-2 py-0.5 rounded-full">
              ...
              <div className="flex flex-col gap-3">
                {ativas.filter(o => o.status === col.id).map(o => (
                  <div key={o.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 shadow-sm hover:border-zinc-700 transition">
                    <h4 className="text-sm font-bold text-zinc-100 leading-tight">{o.titulo}</h4>
                    {o.descricao && <p className="text-[11px] text-zinc-400 mt-2 line-clamp-3 leading-relaxed">{o.descricao}</p>}
                    
                    <div className="mt-4 pt-3 border-t border-zinc-800/60 grid grid-cols-2 gap-2">
                      <div>
                        <p className="text-[9px] text-zinc-600 uppercase tracking-widest">Aporte Disp.</p>
                        <p className="text-xs font-semibold text-zinc-300">{o.investimento_estimado ? formatCurrency(o.investimento_estimado) : 'TBD'}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[9px] text-zinc-600 uppercase tracking-widest">ROI Previsto</p>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      <select 
                        className="text-[10px] bg-zinc-800 border-none outline-none rounded px-1.5 py-1 text-zinc-400 hover:text-zinc-200"
                        value={o.status}
                      ...
                      <span className="text-[9px] text-zinc-600">{formatRelative(o.created_at)}</span>
                    </div>
                  </div>
                ))}
              </div>
