  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-sm font-semibold text-zinc-200">🎯 Objetivos e Resultados Chave (OKRs)</h2>
      ...
      {showForm && (
        <form onSubmit={handleSalvarOkr} className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 space-y-3">
      ...
      {okrs.length === 0 && !showForm ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8"><EmptyState message="Defina os OKRs estratégicos da empresa para o ciclo." /></div>
      ) : (
        <div className="space-y-4">
          {okrs.map(okr => {
            ...
            return (
              <div key={okr.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-700 transition">
                {/* Cabeçalho do Objetivo */}
                <div className="p-5 border-b border-zinc-800/60 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <select className={cn("text-[10px] px-2 py-0.5 rounded-full outline-none font-semibold", 
                          ...
                          okr.status === 'atrasado' ? 'text-red-400 bg-red-400/10' : 'text-zinc-400 bg-zinc-800'
                        )}
                    ...
                    <h3 className="text-base font-bold text-zinc-100 flex items-center gap-2">🎯 {okr.objetivo}</h3>
                  </div>
                  
                  {/* Progresso Geral */}
                  <div className="w-full md:w-48">
                    <div className="flex justify-between items-end mb-1">
                      <span className="text-[10px] text-zinc-500 font-semibold uppercase">Progresso Geral</span>
                      <span className="text-xs font-bold text-emerald-400">{mediaProgresso.toFixed(0)}%</span>
                    </div>
                    <div className="h-2 bg-zinc-950 rounded-full border border-zinc-800 overflow-hidden relative">
                ...
                <div className="p-5 bg-zinc-950">
                  <h4 className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest mb-3">Resultados Chave (KRs)</h4>
                    ...
                    {krs.map(kr => {
                      ...
                      return (
                        <div key={kr.id} className="flex items-center gap-4">
                          <p className="flex-1 text-sm text-zinc-300 font-medium">↳ {kr.resultado_chave}</p>
                          <div className="flex items-center gap-3 shrink-0">
                            <input type="number" 
                              className="w-20 text-center bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-purple-500"
                              value={kr.atual_valor}
                              onChange={e => updateKRProgress(kr.id, parseFloat(e.target.value) || 0)}
                            />
                            <span className="text-xs text-zinc-500 w-16">/ {kr.meta_valor} {kr.unidade}</span>
                            
                            <div className="w-24 h-5 bg-zinc-900 rounded overflow-hidden relative">
                              <div className="absolute top-0 left-0 h-full bg-zinc-700 transition-all" style={{width: `${pct}%`}} />
                  ...
                  {formKR.okr_id === okr.id ? (
                    <form onSubmit={handleSalvarKR} className="mt-4 flex gap-2">
                       ...
                       <button type="button" onClick={() => setFormKR({...formKR, okr_id: ''})} className="text-zinc-500 px-2 text-xs">✕</button>
                    </form>
                  ) : (
