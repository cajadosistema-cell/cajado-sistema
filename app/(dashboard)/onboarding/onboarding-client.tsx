      ...
      <div className="w-full max-w-2xl relative z-10">
          ...
          <div className="bg-surface border border-white/5 rounded-2xl p-8 shadow-2xl space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input id="cnpj" className="input mt-1" placeholder="00.000.000/0001-00" value={form.cnpj} onChange={e => set('cnpj', e.target.value)} />
              </div>
              <div>
                <label className="label">WhatsApp / Telefone</label>
                <input id="telefone" className="input mt-1" placeholder="(11) 99999-9999" value={form.telefone} onChange={e => set('telefone', e.target.value)} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mt-2">
                {SEGMENTOS.map(s => (
                  <button key={s.id} id={`seg-${s.id}`} type="button" onClick={() => set('segmento', s.id)}
                    className={cn(
            ...
            {erro && <div className="text-xs text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-3 py-2">⚠️ {erro}</div>}

            <div className="flex justify-end pt-2">
              <button id="btn-proximo-etapa1" onClick={() => {
        ...
        {etapa === 2 && (
          <div className="bg-surface border border-white/5 rounded-2xl p-8 shadow-2xl space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {PLANOS.map(p => (
                <button key={p.id} id={`plano-${p.id}`} type="button" onClick={() => set('plano', p.id)}
                  className={cn(
                  ...
                  <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full self-start mb-3', p.badgeCor)}>{p.badge}</span>
            ...
            <div className="flex justify-between pt-2">
              <button id="btn-voltar-etapa2" onClick={() => { setErro(''); setEtapa(1) }} className="btn-secondary">← Voltar</button>
              <button id="btn-criar-empresa" onClick={handleCriarEmpresa} disabled={loading} className="btn-primary px-8 min-w-[160px]">
                {loading ? (
        ...
        {etapa === 3 && (
          <div className="bg-surface border border-white/5 rounded-2xl p-10 shadow-2xl text-center space-y-6">
            ...
            <button id="btn-entrar-sistema" onClick={handleFinalizar} className="btn-primary px-12 py-3 text-base font-bold mx-auto">
              Entrar no Sistema →
            </button>
            <p className="text-xs text-fg-disabled">
              Configure mais detalhes em <strong className="text-fg-tertiary">Configurações</strong> a qualquer momento.
            </p>
          </div>
        )}
      </div>
