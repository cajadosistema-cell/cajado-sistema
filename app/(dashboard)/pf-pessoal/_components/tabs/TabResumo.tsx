      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-zinc-300">Gastos por Categoria</h3>
            <button onClick={onNovoGasto} className="btn-ghost text-xs">+ Gasto</button>
          </div>
          {gastosPorCategoria.length === 0 ? (
            <p className="text-xs text-zinc-600 text-center py-8">Nenhum gasto registrado este mês.</p>
          ) : (
                ...
                <div key={cat}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="text-right">
                      ...
                      {orcamento && (
                        <span className="text-xs text-zinc-600 ml-1">/ {formatCurrency(orcamento.valor_limite)}</span>
                      )}
                    </div>
                  </div>
                  {orcamento && (
        ...
        <div className="space-y-4">
          {/* Top 5 maiores gastos */}
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-zinc-300 mb-3">🔝 Maiores Gastos do Mês</h3>
            {top5.length === 0 ? (
              <p className="text-xs text-zinc-600 text-center py-4">Sem dados.</p>
            ) : (
                ...
                {top5.map((g, i) => (
                  <div key={g.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-zinc-300">{g.descricao}</span>
                    </div>
