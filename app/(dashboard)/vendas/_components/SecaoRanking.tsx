  ...
  return (
    <div className="bg-[#111827] border border-white/5 rounded-xl p-5">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-sm font-semibold text-zinc-100">🏆 Ranking de Vendas</h2>
          <p className="text-xs text-zinc-500 mt-0.5">Desempenho por responsável</p>
        </div>
          ...
          {PERIODOS.map(p => (
            <button key={p.id} onClick={() => setPeriodo(p.id as any)}
              className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                periodo === p.id ? 'bg-zinc-700 text-zinc-100' : 'text-zinc-500 hover:text-zinc-300'
              }`}>
              {p.label}
            </button>
          ))}
      ...
      {ranking.length === 0 ? (
        <div className="py-12 text-center">
          <p className="text-sm text-zinc-600">Nenhuma venda registrada neste período</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
              {ranking.slice(0, 3).map((r, i) => (
                <div key={r.id}
                  ...
                  <p className="text-sm font-semibold text-zinc-100 truncate">{r.nome}</p>
                  <p className={`text-xl font-bold mt-1 ${i === 0 ? 'text-amber-400' : 'text-zinc-200'}`}>
                    {formatCurrency(r.total)}
                  </p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-zinc-500">{r.qtd} venda{r.qtd !== 1 ? 's' : ''}</span>
                    <span className="text-xs text-zinc-500">TM: {formatCurrency(r.ticket_medio)}</span>
                  </div>
                  ...
                  {r.pct_meta !== null && (
                    <div className="mt-3">
                      <div className="flex justify-between text-[10px] mb-1">
                        ...
                        <span className={r.pct_meta >= 100 ? 'text-emerald-400' : 'text-zinc-400'}>{r.pct_meta.toFixed(0)}%</span>
                      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-800">
                <th className="text-left py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">#</th>
                <th className="text-left py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Atendente</th>
                <th className="text-right py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Vendas</th>
                <th className="text-right py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Total</th>
                <th className="text-right py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider hidden md:table-cell">Ticket Médio</th>
                <th className="text-right py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider hidden md:table-cell">% do total</th>
                {metas.length > 0 && (
                  <th className="text-right py-2 px-3 text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Meta</th>
                )}
              </tr>
            </thead>
            <tbody>
              {ranking.map((r, i) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-zinc-800/40 hover:bg-white/2 transition-colors">
                    <td className="py-3 px-3 text-zinc-500 text-sm">{medalhas[i] ?? i + 1}</td>
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-zinc-200">{r.nome}</span>
                      </div>
                    </td>
                    <td className="py-3 px-3 text-right text-sm text-zinc-400">{r.qtd}</td>
                    <td className="py-3 px-3 text-right text-sm font-semibold text-zinc-100">{formatCurrency(r.total)}</td>
                    <td className="py-3 px-3 text-right text-sm text-zinc-400 hidden md:table-cell">{formatCurrency(r.ticket_medio)}</td>
                    <td className="py-3 px-3 text-right hidden md:table-cell">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-20 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div className="h-full bg-amber-500/70 rounded-full" style={{ width: `${pctTotal}%` }} />
                        </div>
                        <span className="text-xs text-zinc-500 min-w-[30px]">{pctTotal.toFixed(0)}%</span>
                      </div>
                    </td>
                    {metas.length > 0 && (
